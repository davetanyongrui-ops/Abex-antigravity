...
export function ProjectTimeline() {
    ...
    return (
        <section className="py-24 max-w-7xl mx-auto px-6 lg:px-8 text-left">
            <Tabs defaultValue="2024" className="w-full">
                <div className="flex flex-col lg:flex-row gap-12">
                    {/* Year Selector (Side) */}
                    <div className="lg:w-1/4 shrink-0">
                        <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-6 border-b-2 border-slate-900 pb-2">{t('selectYear')}</h3>
                        <TabsList className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-2 bg-transparent h-auto p-0 gap-2 w-full">
                            {years.map(year => (
                                <TabsTrigger
                                    key={year}
                                    value={year}
                    ...
                    <div className="lg:w-3/4">
                        {years.map(year => (
                            <TabsContent key={year} value={year} className="mt-0 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <div className="flex items-baseline gap-4 mb-8">
                                    <h2 className="text-4xl font-black text-slate-900 uppercase tracking-tighter">{t('installationsTitle', { year })}</h2>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    {projectData[year]?.map((project, idx) => (
                                        <div key={`${project.name}-${idx}`} className="group bg-white rounded-xl border border-slate-200 p-6 hover:border-blue-500 hover:shadow-md transition-all flex flex-col gap-5">
